...
flatten prefix (A.String x    ) = HM.singleton prefix x
flatten prefix (A.Bool   True ) = HM.singleton prefix "true"
flatten prefix (A.Bool   False) = HM.singleton prefix "false"
flatten prefix A.Null           = HM.singleton prefix "null"
flatten prefix (A.Object xs) =
    HM.unions [ flatten (mergePath prefix k) v | (k, v) <- HM.toList xs ]
flatten prefix (A.Array xs) = HM.unions
    [ flatten (mergePath prefix $ T.pack $ show k) v
