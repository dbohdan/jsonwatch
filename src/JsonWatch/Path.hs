...
data PathElement = IntIndex Int | TextKey T.Text deriving (Eq, Ord, Show)
...
addText t pels = TextKey t : pels

empty :: Path
empty = []

formatPath :: Path -> T.Text
formatPath pels = T.concat $ fmap f pels
  where
    f (IntIndex i) = T.append "." $ T.pack $ show i
    f (TextKey t) = case T.find (\c -> c == ' ') t of
